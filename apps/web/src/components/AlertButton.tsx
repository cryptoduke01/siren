...
export function TokenAlertButton({ mint, symbol, price }: { mint: string; symbol: string; price?: number }) {
  const { getTokenAlerts, addTokenAlert, removeTokenAlert } = useAlertStore();
  const [open, setOpen] = useState(false);
  const alerts = getTokenAlerts(mint);
  const hasAlert = alerts.length > 0;

  const handleAdd = (kind: "pump" | "dump", pct: number) => {
    hapticLight();
    addTokenAlert(mint, kind, pct, price);
    setOpen(false);
  };
    ...
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div
            className="absolute right-0 top-full mt-1 z-20 rounded-[6px] border p-2 min-w-[140px]"
            style={{ background: "var(--bg-elevated)", borderColor: "var(--border-default)" }}
            onClick={(e) => e.stopPropagation()}
          >
            <p className="font-body text-[10px] mb-2" style={{ color: "var(--text-3)" }}>Alert when {symbol}</p>
            <button type="button" onClick={() => handleAdd("pump", 20)} className="block w-full text-left font-body text-[11px] py-1 hover:bg-[var(--bg-hover)] rounded px-2" style={{ color: "var(--text-1)" }}>
              +20% from now
            </button>
            <button type="button" onClick={() => handleAdd("dump", 20)} className="block w-full text-left font-body text-[11px] py-1 hover:bg-[var(--bg-hover)] rounded px-2" style={{ color: "var(--text-1)" }}>
              -20% from now
            </button>
            {hasAlert && (
              <button type="button" onClick={handleRemove} className="block w-full text-left font-body text-[11px] py-1 mt-1 rounded px-2" style={{ color: "var(--down)" }}>
