...
export default async function OpenGraphImage({ params }: MarketImageProps) {
  ...
  const subtitle =
    market != null
      ? `${market.source === "polymarket" ? "Polymarket" : "Kalshi"} market on Siren${
          typeof market.probability === "number" ? ` · YES ${Math.min(100, Math.max(0, market.probability)).toFixed(1)}c` : ""
        }`
